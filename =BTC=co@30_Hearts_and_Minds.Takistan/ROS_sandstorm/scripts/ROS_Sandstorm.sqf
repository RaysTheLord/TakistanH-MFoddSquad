...
_affectsSmallObjs = false;
_affectsMediumObjs = false;
...
[_endtime, _affectsSmallObjs, _affectsMediumObjs] spawn {
    ...
    while {time <= _endtime} do {
        ...
        if (_affectsSmallObjs or _affectsMediumObjs && hasInterface) then {
...
if (_debug) then {hint "Start leaves"; sleep 1;};
 if (hasinterface) then {[] spawn SS_leaves_Fnc};
...
if (hasinterface) then {deleteVehicle SS_dust_devil_par};
