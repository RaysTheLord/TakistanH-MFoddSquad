...
[{
    ...
    for "_i" from 0 to ((_crews min _numberOfUnits) - 1) do {
        ...
        if ((_units_type select _i) isEqualTo any) then {
            _unit = _group createUnit ["CUP_O_TK_INS_Soldier", _position, [], 0, "CAN_COLLIDE"];
        } else {
            _unit = _group createUnit [_units_type select _i, _position, [], 0, "CAN_COLLIDE"];
        };
